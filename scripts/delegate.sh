...
delegate_tokens() {
  ...
  echo "Delegating ${amount}unyx to ${moniker} (${valoper})..."

  # Prompt for confirmation before broadcasting.
  read -r -p "Proceed to delegate ${amount}unyx to ${moniker}? (y/N) " response
  case "$response" in
    [yY][eE][sS]|[yY])
      nyxd tx staking delegate "$valoper" "$amount"unyx \
        --from "$KEY_NAME" \
        --chain-id "$CHAIN_ID" \
        --node "$NODE_RPC" \
        --fees "$FEE"
      ...
      if [ $? -ne 0 ]; then
        echo "Error: Delegation failed for ${moniker}."
        exit 1
      else
        echo "Delegation to ${moniker} successful. Sleeping for 3 seconds..."
        sleep 3
      fi
      ;;
    *)
      echo "Delegation to ${moniker} skipped."
      ;;
  esac
}
...
echo "Delegation script completed."
